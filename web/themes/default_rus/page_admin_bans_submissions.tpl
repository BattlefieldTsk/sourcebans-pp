{if NOT $permissions_submissions}
	Access Denied!
{else}
	<h3 style="margin-top:0px;">Запросы на блокировку игроков (<span id="subcount">{$submission_count}</span>)</h3>
	Нажмите на нике игрока для просмотра информации о запросе на блокировку.<br /><br />
    <div id="banlist-nav"> 
    {$submission_nav}
    </div>
		...
		{foreach from="$submission_list" item="sub"}
			<tr id="sid_{$sub.subid}" class="opener3 tbl_out" {if $sub.hostname == ""}onclick="xajax_ServerHostPlayers('{$sub.server}', 'id', 'sub{$sub.subid}');"{/if} onmouseout="this.className='tbl_out'" onmouseover="this.className='tbl_hover'">
	            ...
	            <td class="listtable_1" height='16'> 
		            <a href="#" onclick="xajax_SetupBan({$sub.subid});return false;">Заблокировать</a> - 
		            {if $permissions_editsub}
		           		<a href="#" onclick="RemoveSubmission({$sub.subid}, '{$sub.name|stripslashes|stripquotes}', '1');return false;">Убрать в архив</a> -
		           	{/if}
					<a href="index.php?p=admin&c=bans&o=email&type=s&id={$sub.subid}">Связаться</a>
				</td>
			</tr>
					...
					<div class="opener3" width="100%" align="center">
						<table width="90%" cellspacing="0" cellpadding="0" class="listtable">
							...
							<tr align="left">
            					...
            					<td height="16" class="listtable_1">
								{if $sub.sip == ""}
									<i><font color="#677882">IP адрес отсутствует</font></i>
								{else}
									{$sub.sip}
								{/if}
								</td>
							...
							<tr align="left">
            					...
            					<td height="" class="listtable_1" id="sub{$sub.subid}">{if $sub.hostname == ""}<i>Получение имени сервера</i>{else}{$sub.hostname}{/if}</td>
      						</tr>
      						<tr align="left">
							...
							<tr align="left">
								...
								{if $sub.subname == ""}
									<i><font color="#677882">имя отсутствует</font></i>
								{else}
									{$sub.subname}
								{/if}
								</td>
